...
class ParquetOutputWriter(props: Map[String, String], outputFile: Option[File]) extends MetricOutputWriter {
  ...
  case class ParquetOutputProperties(saveMode: SaveMode, path: String, partitionBy: Seq[String], processingTime: String)
  ...
  val processingTime = props.getOrElse("processingTime", "10 seconds")
